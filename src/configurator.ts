...
import { DatabaseConfig, AppConfig } from './types';
...
export async function runConfigurator(): Promise<void> {
      ...
      const keysAnswer = await inquirer.prompt([
        {
          ...
          validate: (input: string) => (input.trim().length > 0 ? true : 'Access Key ID is required'),
        },
        {
          type: 'password',
          name: 'secretAccessKey',
          message: 'Enter AWS Secret Access Key:',
          validate: (input: string) => (input.trim().length > 0 ? true : 'Secret Access Key is required'),
        },
      ]);
  ...
  try {
    clusters = await listRdsClusters(answers.region, { profile, accessKeyId, secretAccessKey });
  } catch (error) {
    ...
    const arnAnswer = await inquirer.prompt([
      {
        ...
        validate: (input: string) => (input.trim().length > 0 ? true : 'Resource ARN is required'),
      },
    ]);
  ...
  if (dbAuthAnswer.dbAuthMethod === 'Secrets Manager') {
    ...
    try {
      secrets = await listSecrets(answers.region, { profile, accessKeyId, secretAccessKey });
    } catch (error) {
      ...
      const secretArnAnswer = await inquirer.prompt([
        {
          ...
          validate: (input: string) => (input.trim().length > 0 ? true : 'Secret ARN is required'),
        },
      ]);
    ...
    const credAnswer = await inquirer.prompt([
      {
        ...
        validate: (input: string) => (input.trim().length > 0 ? true : 'Username is required'),
      },
      {
        type: 'password',
        name: 'password',
        message: 'Enter database password:',
        validate: (input: string) => (input.trim().length > 0 ? true : 'Password is required'),
      },
    ]);
    ...
    console.log('   then run the configurator again and select "Secrets Manager".\n');
  }
  ...
  const databaseAnswer = await inquirer.prompt([
    {
      ...
      validate: (input: string) => (input.trim().length > 0 ? true : 'Database name is required'),
    },
  ]);
